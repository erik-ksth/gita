...
"""
    ...
    try:
        ...
        data = {
            "model": "meta-llama/llama-4-scout-17b-16e-instruct",  # Updated to current Groq vision model
            "messages": messages,
            "max_tokens": 1000,
            "temperature": 0.7
        }
...
def get_frames_from_supabase(video_id: str) -> List[str]:
    ...
    try:
        ...
        if response.data:
            ...
            print(f"Retrieved {len(frame_paths)} frame paths from Supabase for video {video_id}")
            return frame_paths
        else:
...
def analyze_images_from_supabase(image_paths: List[str]) -> str:
    ...
    try:
        ...
        print("Consider using analyze_video_frames_from_supabase() with video_id instead")
